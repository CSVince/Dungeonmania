...
import dungeonmania.entities.rules.RuleFactory;
...
public class EntityFactory {
    ...
    public PositionalEntity createEntity(JSONObject jsonEntity) {
    ...
    public void spawnSpider(Game game) {
        ...
        availablePos = availablePos
            .stream()
            .filter(p -> !map.getEntities(p)
                            .stream()
                            .anyMatch(e -> (e instanceof Boulder || e instanceof Player || e instanceof Enemy)))
            .collect(Collectors.toList());
        if (availablePos.size() == 0) return;
    ...
    public OlderPlayer buildOlderPlayer(Position pos, JSONArray actions) {
        double playerHealth = config.optDouble("player_health", Player.DEFAULT_HEALTH);
        double playerAttack = config.optDouble("player_attack", Player.DEFAULT_ATTACK);
        return new OlderPlayer(pos, playerHealth, playerAttack, actions);
    }
    ...
    public Hydra buildHydra(Position pos) {
        ...
        double hydraIncrease = config.optDouble("hydra_health_increase_amount", Hydra.DEFAULT_HEALTH_INCREASE_AMOUNT);
        double hydraIncreaseRate = config.optDouble("hydra_health_increase_rate", Hydra.DEFAULT_HEALTH_INCREASE_RATE);
        return new Hydra(pos, hydraHealth, hydraAttack, hydraIncrease, hydraIncreaseRate);
    }
    ...
    public Sceptre buildSceptre() {
        int duration = config.optInt("mind_control_duration", Sceptre.DEFAULT_DURATION);
        return new Sceptre(duration);
    }

    public MidnightArmour buildMidnightArmour() {
        double midnightArmourAttack = config.optDouble("midnight_armour_attack", MidnightArmour.DEFAULT_ATTACK);
        double midnightArmourDefence = config.optDouble("midnight_armour_defence", MidnightArmour.DEFAULT_DEFENCE);
        return new MidnightArmour(midnightArmourDefence, midnightArmourAttack);
    }

    private PositionalEntity constructEntity(JSONObject jsonEntity, JSONObject config) {
        ...
        case "portal":
            return new StandardPortal(pos, ColorCodedType.valueOf(jsonEntity.getString("colour")));
        case "sword":
        ...
        case "wire":
            return new Wire(pos);
        case "light_bulb_off":
            return new LightBulb(pos, new RuleFactory().createRule(jsonEntity.getString("logic")));
        case "switch_door":
            return new SwitchDoor(pos, new RuleFactory().createRule(jsonEntity.getString("logic")));
        case "time_turner":
            return new TimeTurner(pos);
        case "time_travelling_portal":
            return new TimeTravellingPortal(pos);
        case "older_player":
            JSONArray actions = jsonEntity.getJSONArray("actions");
            return buildOlderPlayer(pos, actions);
        default:
            return null;
        }
    }
