...
public class PortalsTest {
    ...
    public void testNoEffectOnZombie() {
        ...
        // This is the position of the destination portal (where the zombie
        // would end up IF it teleported)
        Position portalPosition = new Position(700, 100);
    ...
    @Test
    @Tag("7-7")
    @DisplayName("Test portal plays a factor in mercenary path")
    public void testPortalDijkstra() {
        DungeonManiaController controller = new DungeonManiaController();
        DungeonResponse res = controller.newGame(
            "d_portalTestDijkstra", "c_PortalsTest_testNoEffectOnZombie");
        for (int i = 0; i < 5; i++) {
            res = controller.tick(Direction.DOWN);
